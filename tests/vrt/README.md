...
The main-branch baseline is overwritten at the fixed, extensionless R2 object key `visual-regression/nicorandompicker-v2/baseline`. Keeping a single object prevents baseline history from consuming ever more storage, while omitting the `.zst` suffix avoids [Wrangler's known stale reads for overwritten `.zst` keys](https://github.com/cloudflare/developer-platform/issues/53). The local archive remains zstd-compressed; only its R2 object key is extensionless.
