...
pub(super) async fn handle(State(state): State<AppState>, Json(params): Json<SearchParams>) -> ApiResult<Vec<VideoSearchResult>> {
    ...
    tracing::info!(
        ...
        tag_len = params.tag_name.len(),
        has_view_range = params.view_count_min.is_some() || params.view_count_max.is_some(),
        has_time_range = params.start_time_from.is_some() || params.start_time_to.is_some(),
