...
pub(super) trait ApiState: Clone + Send + Sync + 'static {
    ...
    fn authenticated_rate_limit(&self, key: String) -> impl Future<Output = Result<bool, Self::Err>> + Send;
    fn anonymous_rate_limit(&self, key: String) -> impl Future<Output = Result<bool, Self::Err>> + Send;
    fn turnstile_verify_burst_rate_limit(&self, key: String) -> impl Future<Output = Result<bool, Self::Err>> + Send;
    fn turnstile_verify_client_rate_limit(&self, key: String) -> impl Future<Output = Result<bool, Self::Err>> + Send;
}
...
impl ApiState for AppState {
    ...
    async fn authenticated_rate_limit(&self, key: String) -> Result<bool, Self::Err> {
        Ok(self.authenticated_rate_limiter.limit(key).await?.success)
    }

    async fn anonymous_rate_limit(&self, key: String) -> Result<bool, Self::Err> {
        Ok(self.anonymous_rate_limiter.limit(key).await?.success)
    }

    async fn turnstile_verify_burst_rate_limit(&self, key: String) -> Result<bool, Self::Err> {
        Ok(self.turnstile_verify_burst_rate_limiter.limit(key).await?.success)
    }

    async fn turnstile_verify_client_rate_limit(&self, key: String) -> Result<bool, Self::Err> {
...
mod tests {
    ...
    impl ApiState for TestState {
        ...
        async fn authenticated_rate_limit(&self, _key: String) -> Result<bool, Self::Err> {
            Ok(true)
        }

        async fn anonymous_rate_limit(&self, _key: String) -> Result<bool, Self::Err> {
            Ok(true)
        }

        async fn turnstile_verify_burst_rate_limit(&self, _key: String) -> Result<bool, Self::Err> {
            Ok(true)
        }

        async fn turnstile_verify_client_rate_limit(&self, _key: String) -> Result<bool, Self::Err> {
            Ok(true)
        }
    }
